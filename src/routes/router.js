...
router.post('/api/user/:userId', async(req, res) => {
    ...
    try {
        ...
        const user = await User.findByIdAndUpdate(
            userId, { username, password, role }, { new: true }
        );
        res.redirect('/report');
    } catch (error) {
...
router.put('/api/profiles/:id', async(req, res) => {
    try {
        ...
        const profile = await Profile.findByIdAndUpdate(
            req.params.id, { name, fullname, age, salary, address, quote }, { new: true }
        );
...
router.post('/api/v2/user/:userId', async(req, res) => {
    ...
    try {
        ...
        const profile = await Profile.findOneAndUpdate({ user: userId }, { name, fullname, age, salary, address, quote }, { new: true });
...
router.post('/api/timesheets/:id', async(req, res) => {
    try {
        ...
        const timesheet = await Timesheet.findByIdAndUpdate(
            req.params.id, { workType, workName, startTime, endTime, status, updatedAt: Date.now() }, { new: true }
        );
